---
title: "Mapping Sodalink Model to a Neo4j property graph"
parent: "Mapping to a graph store"
layout: default
---

# Mapping Sodalink Model to a Neo4j property graph

This section describes how a Neo4j database is mapped to the Sodalink Model. 
...
Sodalink Model defines a typology of nodes, all of which inherit from [NamedThing](../docs/NamedThing).
...
> **Note:** While the CURIE for a property is `sodalink:name` that does not necessarily mean the property
> name has to be `sodalink:name` in Neo4j. Instead, the prefix part of the property can be omitted such that
...
The Sodalink Model class name in CamelCase MUST be used for the `category` field.
...
For this reason we use [category](../docs/category) as the property name in Sodalink Model.
...
Sodalink Model includes a hierarchy of [Association](../docs/Association.html).
