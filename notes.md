...
The Sodalink Model is defined as a YAML. Any changes or edits are to be made to this YAML.
...
The generation of Sodalink Model artifacts requires Python 3.7+ because it makes extensive use of 
...
Three `secrets` are used by sodalink pull request workflows:
...
To generate Sodalink Model JSON-LD context,
```bash
make context.jsonld
```
